 ...
 * @author Udo Kastilan
 */
public class Log4JAdapter implements Log.Provider {
	@Override
	public void init(File configFolder) {
		String env = Environment.getInstance().getHint();
		String[] test = { "log4j-" + env + ".properties", "log4j-" + env + ".xml", "log4j.properties", "log4j.xml" };
		boolean found = false;
		...
		if (!found) {
			System.out.println(
					"No explicit log configuration found; please use log4j default schematics for configuration (e.g. a log4j.properties in the class path)");
		}
	}
